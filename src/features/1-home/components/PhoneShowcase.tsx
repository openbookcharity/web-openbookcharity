...
export function PhoneShowcase() {
  return (
    <div className="relative mx-auto w-full max-w-md">
      <div className="absolute -left-6 top-24 hidden size-3 rounded-full bg-primary/50 lg:block" />
      <div className="absolute -right-4 -top-4 z-20 rounded-2xl bg-navy px-5 py-4 text-navy-foreground shadow-float">
        <p className="flex items-center gap-2 text-eyebrow text-navy-foreground/70">
          <span className="size-2 rounded-full bg-mint" /> Health score
        </p>
        <p className="mt-1 font-display text-3xl">78</p>
        <p className="text-xs text-navy-foreground/70">Active · 12 action items</p>
      </div>
      ...
      <div className="rounded-[2.5rem] border border-border bg-card p-3 shadow-float">
        <div className="overflow-hidden rounded-[2rem] bg-sand">
          ...
          <div className="relative">
            <img
              src={heroImage}
              alt="A mother holding her child outside her home after a funded project"
              width={900}
              height={1200}
          ...
          <div className="bg-card px-5 py-4">
            <p className="font-display text-lg leading-tight text-navy">
              Help Grace's Grandmother Start A Maize Business
            </p>
            <p className="mt-1 text-xs text-muted-foreground">
              Grace is being raised by her grandmother.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="absolute -bottom-6 -left-4 z-20 w-56 rounded-2xl bg-card p-4 shadow-float">
        <p className="text-sm font-semibold text-navy">Clement's Freedom</p>
        <div className="mt-3 h-1.5 w-full rounded-full bg-muted">
          <div className="h-full w-[73%] rounded-full bg-primary" />
        </div>
        <div className="mt-2 flex justify-between text-[11px] text-muted-foreground">
          <span>73% funded</span>
          <span>$2,190 raised</span>
        </div>
      </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold leading-tight text-navy">
              Impact update received
            </p>
            <p className="mt-1 text-xs text-muted-foreground">
              A donor can see what changed.
            </p>
          </div>
        </div>
      </div>
